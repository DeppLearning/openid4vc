...
use oid4vc_core::{DidMethod, Subject, SubjectSyntaxType};
use oid4vc_manager::{methods::iota_method::IotaSubject, ProviderManager, RelyingPartyManager};
use siopv2::{
...
async fn test_implicit_flow_iota() {
    ...
    let client_id = subject.identifier().unwrap();
    ...
    let request: AuthorizationRequest = RequestUrl::builder()
        ...
        .client_metadata(
            ClientMetadata::default()
                .with_subject_syntax_types_supported(vec![SubjectSyntaxType::Did(
                    DidMethod::from_str("did:iota").unwrap(),
                )])
                .with_id_token_signing_alg_values_supported(vec!["EdDSA".to_string()]),
        )
        // .claims(
    ...
    let request_url_by_value = RequestUrl::Request(Box::new(request.clone()));
    dbg!(request_url_by_value.to_string());

    let encoded = relying_party_manager.encode(&request).unwrap();
    dbg!(encoded.clone());
    ...
    let request_url = RequestUrl::builder()
        ...
        .build()
        .unwrap();
    dbg!(request_url.to_string());

    // The Provider obtains the reuquest url either by a deeplink or by scanning a QR code. It then validates the
    // request. Since in this case the request is a JWT, the provider will fetch the request by sending a GET
    // request to mock server's `request_uri` endpoint.
    let request = provider_manager.validate_request(request_url).await.unwrap();
    let request_by_value = provider_manager.validate_request(request_url_by_value).await.unwrap();
    ...
    let response_by_value = provider_manager
        .generate_response(request_by_value, Default::default(), None, None)
        .unwrap();
    dbg!(&response_by_value);
